...
def nucleus_sampling(logits, p=0.8):
    ...
    Returns:
        next_token: index of the next predicted token. Shape: (1,)
    """
    # 1. Transform the logits into probabilities using softmax
    probs = F.softmax(logits, dim=-1)

    # 2. Sort the probabilities in descending order
    sorted_probs, sorted_indices = torch.sort(probs, descending=True)

    # 3. Compute the cumulative probabilities
    cumulative_probs = torch.cumsum(sorted_probs, dim=-1)

    # 4. Mask tokens whose cumulative probability exceeds p
    cutoff_index = (cumulative_probs > p).nonzero(as_tuple=True)[0][0] + 1
    sorted_indices_to_keep = sorted_indices[:cutoff_index]

    # 5. Mask the probabilities that are not in the selected set
    mask = torch.ones_like(probs, dtype=torch.bool)
    mask[sorted_indices_to_keep] = 0
    probs[mask] = 0

    # Check if all probabilities are zero
    if probs.sum() == 0:
        # Fall back to uniform sampling across all tokens
        probs = torch.ones_like(probs) / probs.size(0)
    else:
        # Re-scale the remaining probabilities
        probs = probs / probs.sum()

    # 6. Sample from the re-scaled distribution
...
def main(args):
    ...
    if args.mode == "train":
        ...
        plt.legend()
        plt.savefig(
            "attn_%s_err_rate.png" % (str(args.use_attn),),
            bbox_inches="tight",
        )
    else:
        print("Testing...")
