...
class ConvBlock(nn.Module):
    def __init__(
        ...
        # Define the convolutional layer with the specified kernel size and padding
        self.conv2d = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=padding)

        # Max pooling layer with kernel size 2x2 and stride 2 (if maxpool is enabled)
...
class CNN(nn.Module):
    def __init__(self, dropout_prob, maxpool=True, batch_norm=True, conv_bias=True):
        ...
        self.conv1 = ConvBlock(channels[0], channels[1], kernel_size=3, padding=1, maxpool=maxpool,
                               batch_norm=batch_norm, dropout=dropout_prob)
        self.conv2 = ConvBlock(channels[1], channels[2], kernel_size=3, padding=1, maxpool=maxpool,
                               batch_norm=batch_norm, dropout=dropout_prob)
        self.conv3 = ConvBlock(channels[2], channels[3], kernel_size=3, padding=1, maxpool=maxpool,
                               batch_norm=batch_norm, dropout=dropout_prob)

        """ Initialize layers for the MLP block """
        self.fc1 = nn.Linear(channels[-1], fc1_out_dim) # (channels[-1] * 6 * 6, fc1_out_dim) in Q2.1
        self.fc2 = nn.Linear(fc1_out_dim, fc2_out_dim)
        # Add dropout for the MLP block
        self.dropout_mlp = nn.Dropout(p=dropout_prob)

        # Pooling Global Average
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)

        # For Q2.2 initalize batch normalization
        self.batch_norm_mlp = nn.BatchNorm1d(fc1_out_dim) if batch_norm else nn.Identity()
    ...
    def forward(self, x):
        ...
        x = self.batch_norm_mlp(x) # Apply Global Average Pooling (Q2.2)
        x = self.dropout_mlp(x)  # Dropout after ReLU
...
def get_number_trainable_params(model):
    """ Q2.3 Determine the number of trainable parameters of CNNs"""
...
def main():
    ...
    parser.add_argument('-data_path', type=str, default='intel_landscapes_v2.npz',)
    ...
    plot(epochs, train_mean_losses, ylabel='Loss', name='CNN 3 train loss, learning_rate={}'.format(opt.learning_rate))
    savefig(f"Q2_1_train_mean_losses_learning_rate_0_001.png", bbox_inches="tight")
    plot(epochs, valid_accs, ylabel='Accuracy', name='CNN 3 valid accuracy, learning_rate= {}'.format(opt.learning_rate))
    savefig(f"Q2_1_valid_accs_learning_rate_0_001.png", bbox_inches="tight")
